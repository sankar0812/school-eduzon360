...
class ClassController extends Controller
{
    ...
    public function studentpromotion()
    {
        ...
        $classPromotions = DB::table('classpromotions')
            ->join('students', 'students.s_classid', '=', 'classpromotions.cp_to')
        ...
        // Check if the result is not empty before accessing properties

        // foreach ($classPromotions as $promotion) {
        //     $count = DB::table('students')
        //         ->where('s_classid', $promotion->cp_to)
        //         ->where('acdm_year', $fyear)
        //         ->count();
        // }
        ...
        // $classPromotions = DB::table('students')
        //     ->join('classpromotions', 'classpromotions.cp_to', '=', 'students.s_classid')
        //     ->join('class_sections', 'class_sections.id', '=', 'students.s_classid')
        //     // ->join('classpromotions as cp_from', 'cp_from.cp_from', '=', 'class_sections.id')
        //     ->select('acdm_year', 'c_class' )
        //     ->where('acdm_year', $fyear)
        //     ->distinct()
        //     ->get();

        // $count = $classPromotions->count();
        ...
        // dd($classPromotions);
        return view('sections.studentpromotion', compact('studentclass', 'fyear', 'classPromotions', 'promotionct'));
    }
        ...
        try {
            ...
            if ($promotion) {
                ...
                if ($student_ids) {
                    ...
                    return redirect()->back()->with('failed', 'Please Select Student');
                }
            }
        } catch (\Exception $e) {
    ...
    public function promotiondelete(Request $request)
    {
        ...
        try {
            ...
            $id = $request->input('promotionid');
            $getfrom = $request->input('froms');
            $getto = $request->input('tos');
            $lastyear = $request->input('lastyear');
            $acdyear = $request->input('acdyear');

            Student::where(['s_classid' => $getto, 'acdm_year' => $fyear])
                ->update([
                    's_classid' => $getfrom,
                    'acdm_year' => $lastyear
                ]);

            Classpromotion::find($id)->delete();

            Oldstudentclass::where(['oldclass_id' => $getto, 'oldyear' => $acdyear])->delete();
