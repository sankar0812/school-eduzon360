...
<h5 class="fw-bold  mb-3"><span class="text-muted fw-light"></span>Edit Exam Time</h5>
        <div class="card">
            ...
            <div class="card-body">
                @if (auth()->user()->type == 'admin')
                    <form action="{{ url('admin/examtimeupdate', $examtimeedit->id) }}" method="POST"
                        enctype="multipart/form-data">
                    @elseif (auth()->user()->type == 'clerk')
                        <form action="{{ url('clerk/examtimeupdate',$examtimeedit->id) }}" method="POST"
                            enctype="multipart/form-data">
                    @else
                ...
                <div class="form-group row mb-3">
                    ...
                    <div class="col-md-5">
                        <select id="" name="section" class="form-control " autocomplete="off" required>
                            <option hidden selected>select section</option>
                            <option value="FN" {{ $examtimeedit->et_name === 'FN' ? 'Selected' : '' }}>FN</option>
                            <option value="AN" {{ $examtimeedit->et_name === 'AN' ? 'Selected' : '' }}>AN</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group row mb-3">
                    ...
                    <div class="col-md-5">
                        <input type="text" name="time" id="" class="form-control" placeholder="time"
                            value="{{ $examtimeedit->time }}" autocomplete="off">
                    </div>
                </div>
